...
#ifndef GEOMETRY_KERNEL_SRC_PROPERTIES_H_
#define GEOMETRY_KERNEL_SRC_PROPERTIES_H_
...
class Property_array : public Base_property_array {
 public:
  ...
  explicit Property_array(const std::string &name, T t = T()) :
...
class Property {
 ...
 public:
  explicit Property(Property_array<T> *p = NULL) : parray_(p) {}
  ...
  reference operator[](int i) {
    assert(parray_ != NULL);
    return (*parray_) [i]; //NOLINT (clang-tidy has a false positive here)
  }
...
class Property_container {
 public:
  ...
  Property_container(const Property_container &_rhs) {
    operator= (_rhs);
  }
  ...
  template <class T> Property<T> get_or_add(const std::string &name,
      const T t = T()) {
  ...
  // get the type of property by its name.
  // returns typeid(void) if it does not exist.
  ...
  template <class T> void remove(Property<T> *h) {
    std::vector<Base_property_array *>::iterator it = parrays_.begin();
    std::vector<Base_property_array *>::iterator end = parrays_.end();
    for (; it != end; ++it) {
      if (*it == h->parray_) {
        delete *it;
        parrays_.erase(it);
