...
compute_face_normal(Face f) const {
  ...
  if (next_halfedge(h) == hend) {  // face is a triangle
    if (!CGAL::collinear(p0, p1, p2)) {
      ...
      //std::cout << "Surface_mesh:: Triangle has zero area" << std::endl;
      //std::cout << "Surface_mesh:: Taking normal from neighbour" << std::endl;
      // Visit the three neighbor faces until finding a valid normal
      for (int i = 0; i < 3; i++) {
        Halfedge h_neighbour = opposite_halfedge(h);
        Point p0_n = vpoint_[to_vertex(h_neighbour)];
        h_neighbour = next_halfedge(h_neighbour);
        Point p1_n = vpoint_[to_vertex(h_neighbour)];
        h_neighbour = next_halfedge(h_neighbour);
        Point p2_n = vpoint_[to_vertex(h_neighbour)];
        if (!CGAL::collinear(p0_n, p1_n, p2_n)) {
          return CGAL::unit_normal(p0_n, p1_n, p2_n);
        }
        // Go to next halfedge
        h = next_halfedge(h);
      }
    ...
    // TODO
    // If we come here, then we return a unit normal
    std::cerr << "Surface_mesh:: You should never come here. Could not find non-zero neighbour triangle" << std::endl;
    return Normal(1,0,0);
  } else {  // face is a general polygon
    std::cout << "Surface_mesh:: Polygon is not triangle" << std::endl;
    Normal n(0, 0, 0);

    hend = h;
    do {
      n = n+CGAL::normal(p0, p1, p2);
      h  = next_halfedge(h);
      p0 = p1;
      p1 = p2;
    ...
    return n = n / sqrt(n.squared_length());
  }
}
  ...
  if (h.is_valid()) {
    ...
    do {
      if (!is_boundary(h)) {
        ...
        denom = sqrt(v1.squared_length() * v2.squared_length());
        if (denom > std::numeric_limits<Scalar>::min()) {
          ...
          n   = CGAL::cross_product(v1, v2);

          // check whether normal is != 0
          denom = sqrt(n.squared_length());
          if (denom > std::numeric_limits<Scalar>::min()) {
            n  = n * angle / denom;
            nn = nn+n;
          }
...
edge_length(Edge e) const {
  Vec3 v = vpoint_[vertex(e, 0)] - vpoint_[vertex(e, 1)];
...
garbage_collection() {
  ...
  Vertex_property<Vertex> vmap =
    add_vertex_property<Vertex>("v:garbage-collection");
  Halfedge_property<Halfedge> hmap =
