...
bool read_obj(Surface_mesh *mesh, const std::string &filename) {
  ...
  std::vector<Texture_coordinate> all_tex_coords;
  // texture coordinates sorted for halfedges
  std::vector<int> halfedge_tex_idx;
  Surface_mesh::Halfedge_property <Texture_coordinate> tex_coords =
    mesh->halfedge_property<Texture_coordinate> ("h:texcoord");
  ...
  while (in && !feof(in) && fgets(s, 200, in)) {
    ...
    if (s[0] == '#' || isspace(s[0])) {
    ...
    } else if (strncmp(s, "vt ", 3) == 0) {  // texture coordinate
      ...
      if (with_tex_coord) {  // add texture coordinates
...
bool write_obj(const Surface_mesh &mesh, const std::string &filename) {
  ...
  // normals
  Surface_mesh::Vertex_property<Point> normals =
    mesh.get_vertex_property<Point> ("v:normal");
  if (normals) {
    for (Surface_mesh::Vertex_iterator vit = mesh.vertices_begin();
         vit != mesh.vertices_end(); ++vit) {
  ...
  // if so then add
  if (with_tex_coord) {
    Surface_mesh::Halfedge_property<Texture_coordinate> tex_coord =
      mesh.get_halfedge_property<Texture_coordinate> ("h:texcoord");
    for (Surface_mesh::Halfedge_iterator hit = mesh.halfedges_begin();
  ...
  for (Surface_mesh::Face_iterator fit = mesh.faces_begin();
    ...
    Surface_mesh::Vertex_around_face_circulator fvend = fvit;
