...
#ifndef GEOMETRY_KERNEL_SRC_SURFACE_MESH_H_
#define GEOMETRY_KERNEL_SRC_SURFACE_MESH_H_
...
class Surface_mesh {
 ...
 public:  //------------------------------------------------ connectivity types
  ...
  struct Vertex_connectivity {
    /// an outgoing halfedge per vertex
    /// (it will be a boundary halfedge for boundary vertices)
    Halfedge  halfedge_;
  };
 ...
 public:  //---------------------------------------------------- iterator types
  ...
  class Vertex_iterator {
   public:
    /// Default constructor
    explicit Vertex_iterator(Vertex v = Vertex(),
                             const Surface_mesh *m = NULL) : hnd_(v),
      mesh_(m) {
      if (mesh_ && mesh_->garbage()) {
        while (mesh_->is_valid(hnd_) &&
               mesh_->is_deleted(hnd_)) {
          ++hnd_.idx_;
        }
      }
    }
    ...
    Vertex_iterator &operator++() {
      ++hnd_.idx_;
      assert(mesh_);
      while (mesh_->garbage() && mesh_->is_valid(hnd_) &&
             mesh_->is_deleted(hnd_)) {
        ++hnd_.idx_;
      }
      return *this;
  ...
  class Halfedge_iterator {
   public:
    /// Default constructor
    explicit Halfedge_iterator(Halfedge h = Halfedge(),
                               const Surface_mesh *m = NULL) : hnd_(h),
      mesh_(m) {
      if (mesh_ && mesh_->garbage()) {
        while (mesh_->is_valid(hnd_) &&
               mesh_->is_deleted(hnd_)) {
  ...
  class Edge_iterator {
   public:
    /// Default constructor
    explicit Edge_iterator(Edge e = Edge(), const Surface_mesh *m = NULL) :
      hnd_(e), mesh_(m) {
      if (mesh_ && mesh_->garbage()) {
        while (mesh_->is_valid(hnd_) &&
               mesh_->is_deleted(hnd_)) {
  ...
  class Face_iterator {
   public:
    /// Default constructor
    explicit Face_iterator(Face f = Face(), const Surface_mesh *m = NULL) :
      hnd_(f), mesh_(m) {
      if (mesh_ && mesh_->garbage()) {
        while (mesh_->is_valid(hnd_) &&
               mesh_->is_deleted(hnd_)) {
 ...
 public:  //------------------------ containers for C++11 range-based for loops
  ...
  class Vertex_container {
   public:
    Vertex_container(Vertex_iterator _begin, Vertex_iterator _end) :
  ...
  class Halfedge_container {
   public:
    Halfedge_container(Halfedge_iterator _begin, Halfedge_iterator _end) :
      begin_(_begin), end_(_end) {}
  ...
  class Edge_container {
   public:
    Edge_container(Edge_iterator _begin, Edge_iterator _end) :
      begin_(_begin), end_(_end) {}
  ...
  class Face_container {
   public:
    Face_container(Face_iterator _begin, Face_iterator _end) :
      begin_(_begin), end_(_end) {}
 ...
 public:  //-------------------------------------------------- circulator types
  ...
  class Vertex_around_vertex_circulator {
   public:
    /// default constructor
    explicit Vertex_around_vertex_circulator(const Surface_mesh *m = NULL,
        Vertex v = Vertex()) : mesh_(m),
      active_(true) {
      if (mesh_) {
  ...
  class Halfedge_around_vertex_circulator {
   public:
    /// default constructor
    explicit Halfedge_around_vertex_circulator(const Surface_mesh *m = NULL,
        Vertex v = Vertex())
  ...
  class Face_around_vertex_circulator {
   public:
    ...
    explicit Face_around_vertex_circulator(const Surface_mesh *m = NULL,
                                           Vertex v = Vertex())
      : mesh_(m), active_(true) {
      if (mesh_) {
        halfedge_ = mesh_->halfedge(v);
  ...
  class Vertex_around_face_circulator {
   public:
    /// default constructor
    explicit Vertex_around_face_circulator(const Surface_mesh *m = NULL,
                                           Face f = Face())
      : mesh_(m), active_(true) {
      if (mesh_) {
  ...
  class Halfedge_around_face_circulator {
   public:
    /// default constructur
    Halfedge_around_face_circulator(const Surface_mesh *m = NULL,
                                    Face f = Face())
      : mesh_(m), active_(true) {
      if (mesh_) {
        halfedge_ = mesh_->halfedge(f);
 ...
 public:  //------------------------------------------------- memory management
  ...
  /// return whether vertex \c v is valid, i.e. the index is stores it
  ...
  bool is_valid(Halfedge h) const {
    return (0 <= h.idx()) && (h.idx() < static_cast<int>(halfedges_size()));
  }
  /// return whether edge \c e is valid, i.e. the index is stores it
  /// within the array bounds.
  bool is_valid(Edge e) const {
    return (0 <= e.idx()) && (e.idx() < static_cast<int>(edges_size()));
  }
  /// return whether face \c f is valid, i.e. the index is stores it
  /// within the array bounds.
  bool is_valid(Face f) const {
    return (0 <= f.idx()) && (f.idx() < static_cast<int>(faces_size()));
  }
 ...
 public:  //-------------------------------------------- low-level connectivity
  ...
  /// start vertex of \c h. it is the next halfedge of the opposite
  /// halfedge of \c h.
  ...
  /// returns whether h is a boundary halfege, i.e., if its face does not
  /// exist.
  ...
  /// returns the face incident to the \c i'th halfedge of edge \c e.
  /// \c i has to be 0 or 1.
  ...
  /// returns whether \c f is a boundary face, i.e., it one of its edges is a
  /// boundary edge.
 ...
 public:  //------------------------------------------------- property handling
  ...
  template <class T> Vertex_property<T> add_vertex_property(
    const std::string &name, const T t = T()) {
    return Vertex_property<T>(vprops_.add<T>(name, t));
  }
  /** add a halfedge property of type \c T with name \c name and default value \c t.
   fails if a property named \c name exists already, since the name has to be unique.
   in this case it returns an invalid property */
  template <class T> Halfedge_property<T> add_halfedge_property(
    const std::string &name, const T t = T()) {
  ...
  /** add a edge property of type \c T with name \c name and default value \c t.
   fails if a property named \c name exists already, since the name has to be unique.
   in this case it returns an invalid property */
  template <class T> Edge_property<T> add_edge_property(
    const std::string &name, const T t = T()) {
  ...
  /** add a face property of type \c T with name \c name and default value \c t.
   fails if a property named \c name exists already, since the name has to be unique.
   in this case it returns an invalid property */
  template <class T> Face_property<T> add_face_property(
    const std::string &name, const T t = T()) {
  ...
  /** add a mesh property of type \c T with name \c name and default value \c t.
   fails if a property named \c name exists already, since the name has to be unique.
   in this case it returns an invalid property */
  template <class T> Mesh_property<T> add_mesh_property(
    const std::string &name, const T t = T()) {
  ...
  template <class T> Mesh_property<T> get_mesh_property(
    const std::string &name) const {
  ...
  template <class T> Vertex_property<T> vertex_property(
    const std::string &name, const T t = T()) {
  ...
  /** if a halfedge property of type \c T with name \c name exists, it is returned.
   otherwise this property is added (with default value \c t) */
  template <class T> Halfedge_property<T> halfedge_property(
    const std::string &name, const T t = T()) {
  ...
  otherwise this property is added (with default value \c t) */
  template <class T> Mesh_property<T> mesh_property(
    const std::string &name, const T t = T()) {
 ...
 public:  //---------------------------------------- geometry-related functions
  ...
  /// compute vertex normals by calling compute_vertex_normal(Vertex) for each
  /// vertex.
...
//=============================================================================
#endif  // GEOMETRY_KERNEL_SRC_SURFACE_MESH_H_
//=============================================================================
