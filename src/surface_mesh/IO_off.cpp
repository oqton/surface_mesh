...
bool read_off_ascii(Surface_mesh *mesh,
  ...
  if (has_texcoords) {
    texcoords = mesh->vertex_property<Texture_coordinate>("v:texcoord");
  }
  if (has_colors) {
  ...
  // #Vertice, #Faces, #Edges
  items = fscanf(in, "%d %d %d\n", reinterpret_cast<int *>(&nV),
                 reinterpret_cast<int *>(&nF), reinterpret_cast<int *>(&nE));
  mesh->clear();
  ...
  for (i = 0; i < nV && !feof(in); ++i) {
    ...
    assert(items == 3);
    v = mesh->add_vertex(Point(p[0], p[1], p[2]));
    lp += nc;
    ...
    if (has_texcoords) {
      ...
      assert(items == 2);
      texcoords[v] = Texture_coordinate(t[0], t[1], 0);
      lp += nc;
    }
  }
  ...
  for (i = 0; i < nF; ++i) {
    ...
    // #vertices
    items = sscanf(lp, "%d%n", reinterpret_cast<int*>(&nV), &nc);
    assert(items == 1);
    vertices.resize(nV);
    lp += nc;
    ...
    // indices
    for (j = 0; j < nV; ++j) {
      items = sscanf(lp, "%d%n", reinterpret_cast<int*>(&idx), &nc);
      assert(items == 1);
...
bool write_off(const Surface_mesh &mesh, const std::string &filename) {
  ...
  bool  has_colors = false;
  Surface_mesh::Vertex_property<Normal> normals =
    mesh.get_vertex_property<Normal>("v:normal");
  Surface_mesh::Vertex_property<Texture_coordinate>  texcoords =
    mesh.get_vertex_property<Texture_coordinate>("v:texcoord");
  Surface_mesh::Vertex_property<Color> colors =
    mesh.get_vertex_property<Color>("v:color");
  if (normals) {
    has_normals = true;
  }
  if (texcoords) {
